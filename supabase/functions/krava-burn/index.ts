...
const KRAVA_BASE_URL = "https://privyai.ch";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const platform = createPrivyPlatformClient({ baseUrl: KRAVA_BASE_URL, appKey });
    const { userToken } = await platform.users.getOrCreate(user.id);
    const client = createPrivyClient({ baseUrl: KRAVA_BASE_URL, getToken: () => userToken });
    ...
    console.error("krava-burn error", e);
    return new Response(
